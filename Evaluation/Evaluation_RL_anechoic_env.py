...
dir_data = "/fldr_data"
dir_hrtf = "/fldr_hrtf"  # TESTING ENVIRONMENT (has to be anechoic here)
dir_model = "/fldr_weights"  # TRAINING ENVIRONMENT
name_model = "name_weights.pth"  # TRAINING ENVIRONMENT
name_sampling_file = "samplingfile_name.txt"  # TESTING ENVIRONMENT (has to be anechoic here)
...
memory_capacity = 2000  # number of experiences to store per location said 20000 first?
...
epsilon_zero = 1  # starting value of epsilon (exploration rate)
...
projectname = 'Snellius_HRTF_wRMSnorm_anechoic_4lyr_MemCap' + '_BatSi_' + str(batch_size) + '_Disc' + str(
    gamma) + '_RewOpt' + str(reward_optimal_choice) + '_RewSOpt' + str(
    reward_suboptimal_choice) + '_RewBad' + str(reward_bad_choice) + '_LR' + str(learning_rate)
